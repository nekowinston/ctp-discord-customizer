...
var stylus = (function () {
	...
	require.register('lexer.js', function (module, exports, require) {
		...
		Lexer.prototype = {
			...
			namedop: function () {
				var captures, tok;
				if (
					(captures = /^(not|and|or|is a|is defined|isnt|is not|is)(?!-)\b([ \t]*)/.exec(this.str))
				) {
					var op = captures[1];
					this.skip(captures);
					if (this.isPartOfSelector()) {
			...
			color: function () {
				return (
					this.rrggbbaa() || this.rrggbb() || this.rgba() || this.rgb() || this.nn() || this.n()
				);
			},
			n: function () {
				var captures;
	...
	require.register('nodes/hsla.js', function (module, exports, require) {
		...
		HSLA.prototype.toString = function () {
			return (
				'hsla(' + this.h + ',' + this.s.toFixed(0) + '%,' + this.l.toFixed(0) + '%,' + this.a + ')'
			);
		};
		...
		Parser.prototype = {
			...
			looksLikeSelector: function (fromProperty) {
				...
				while (
					...
					('newline' == this.lookahead(i + 1).type || ',' == this.lookahead(i + 1).type)
				)
					i += 2;
				while (this.isSelectorToken(i) || ',' == this.lookahead(i).type) {
					...
					if (
						('=' == this.lookahead(i).type || 'function' == this.lookahead(i).type) &&
						'{' == this.lookahead(i + 1).type
					)
						return false;
					if (
				...
				while (
					!~['indent', 'outdent', 'newline', 'for', 'if', ';', '}', 'eos'].indexOf(
						this.lookahead(i).type
					)
				)
					++i;
			...
			relational: function () {
				var op,
					node = this.range();
				while (
					(op = this.accept('>=') || this.accept('<=') || this.accept('<') || this.accept('>'))
				) {
					this.operand = true;
					if (!node) this.error('illegal unary "' + op + '", missing left-hand operand');
	...
	require.register('visitor/compiler.js', function (module, exports, require) {
		...
		Compiler.prototype.visitKeyframes = function (node) {
			...
			this.buf += this.out(
				'@' + prefix + 'keyframes ' + this.visit(node.val) + (this.compress ? '{' : ' {\n'),
				node
			);
			this.keyframe = true;
			++this.indents;
		...
		Compiler.prototype.visitExpression = function (expr) {
			...
			nodes.forEach(function (node, i) {
				...
				if (last) return;
				var space =
					self.isURL || (self.isCondition && (')' == nodes[i + 1] || '(' == node)) ? '' : ' ';
	...
	require.register('visitor/evaluator.js', function (module, exports, require) {
		...
		function importFile(node, file, literal) {
			...
			try {
				...
				if (literal && this.includeCSS && this.resolveURL) {
					this.warn(
						'ParseError: ' + file + ':' + line + ':' + column + '. This file included as-is'
					);
					return literal;
				} else {
					err.filename = file;
		...
		Evaluator.prototype.populateGlobalScope = function () {
			...
			scope.add(
				new nodes.Ident(
					'embedurl',
					new nodes.Function('embedurl', require('../functions/url')({ limit: false }))
				)
			);
		...
		Evaluator.prototype.visitFunction = function (fn) {
			...
			if (local)
				this.warn(
					'local ' + local.nodeName + ' "' + fn.name + '" previously defined in this scope'
				);
		...
		Evaluator.prototype.visitIf = function (node) {
			...
			if (
				...
				!node.postfix &&
				block.node &&
				~['group', 'atrule', 'media', 'supports', 'keyframes'].indexOf(block.node.nodeName)
			) {
		...
		Evaluator.prototype.invokeFunction = function (fn, args, content) {
			...
			scope.add(
				new nodes.Ident('mixin', this.ret ? nodes.no : new nodes.String(mixinBlock.nodeName))
			);
			if (this.property) {
